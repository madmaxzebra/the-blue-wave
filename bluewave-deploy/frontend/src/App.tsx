...
function MailCheckLink({ email }: { email: string }) {
  ...
  const checkMail = async () => {
    ...
    try {
      ...
      if (!r.ok) {
        setResult(`Backend returned ${r.status}. Restart the tunnel.`);
        return;
      }
      const data = await r.json();
      setResult(data.resendConfigured ? 'YES ✓ Mail is configured.' : 'NO ✗ Add RESEND_API_KEY or SMTP credentials to backend/.env');
    } catch {
      setResult('Could not reach backend. Is the tunnel running?');
    } finally {
      setLoading(false);
    }
  };
  const sendTest = async () => {
    if (!email?.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
      setResult('Enter your email above first, then click Send test email.');
      return;
    }
    setLoading(true);
    setResult(null);
    const base = getApiBase();
    try {
      const r = await fetch(`${base}/api/test-email`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email.trim() }) });
      const data = await r.json();
      setResult((data.ok || data.sent) ? '✓ Test email sent! Check your inbox.' : (data.message || 'Failed. Check backend terminal.'));
    } catch {
      setResult('Could not reach backend. Is the tunnel running?');
    } finally {
      setLoading(false);
  ...
  return (
    <p style={{ marginTop: '1rem', fontSize: '0.9rem' }}>
      <button type="button" onClick={checkMail} disabled={loading} className="mail-check-link" style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', textDecoration: 'underline', color: 'rgba(255,255,255,0.95)', font: 'inherit' }}>
        {loading ? '…' : 'Mail setup check →'}
      </button>
      {' · '}
      <button type="button" onClick={sendTest} disabled={loading} className="mail-check-link" style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', textDecoration: 'underline', color: 'rgba(255,255,255,0.95)', font: 'inherit' }}>
        {loading ? '…' : 'Send test email →'}
      </button>
      {result && <span style={{ display: 'block', marginTop: '0.5rem', color: result.startsWith('✓') || result.startsWith('YES') ? '#86EFAC' : result.startsWith('NO') ? '#FCA5A5' : '#FCD34D' }}>{result}</span>}
    </p>
  );
}
