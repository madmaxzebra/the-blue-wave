...
app.get('/api/health', (req, res) => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log('[Health] resendConfigured:', configured, '| usesManus:', usesManus);
  }
  res.json({ ok: true, resendConfigured: configured || usesManus });
});
...
app.post('/api/subscribe', async (req, res) => {
  ...
  try {
    ...
    const msg = welcomeSent ? 'Thanks! Check your inbox for the confirmation.' : 'Thanks for subscribing! Email failed – use Gmail App Password in backend/.env (see MAIL-SETUP.md)';
...
app.listen(PORT, () => {
  ...
  if (hasResend) {
    ...
    console.log('[Blue Wave] Mail: Resend (HTTP API) from', fromAddr);
    console.log('[Blue Wave] To send to real addresses: verify domain in https://resend.com/domains');
  } else if (hasSmtp) {
    console.log('[Blue Wave] Mail: SMTP', process.env.SMTP_HOST === 'smtp.gmail.com' ? '(Gmail)' : '');
    ...
    console.warn('[Blue Wave] Mail not configured. Add RESEND_API_KEY to backend/.env and restart');
  }
});
