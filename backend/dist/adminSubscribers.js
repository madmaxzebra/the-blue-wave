...
function renderHtml(payload) {
    const rows = payload.subscribers
        .map((s) => `<tr><td>${escapeHtml(s.email)}</td><td>${escapeHtml(formatWhen(s.createdAt))}</td></tr>`)
        .join('');
    return `<!DOCTYPE html>
<html lang="en">
<head>
...
async function handleAdminSubscribers(req, res) {
    ...
    const publicDisplayCount = await (0, subscriberCounter_1.getPublicSubscriberCount)();
    const updatedAt = new Date().toLocaleString('en-US', { timeZone: 'America/Curacao' });
    const adminKey = (0, adminAuth_1.readSubscriberAdminKey)(req);
    const payload = {
        ok: true,
