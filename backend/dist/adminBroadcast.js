...
exports.handleAdminBroadcastGet = handleAdminBroadcastGet;
exports.handleAdminBroadcastPost = handleAdminBroadcastPost;
const broadcastToSubscribers_1 = require("./broadcastToSubscribers");
...
function renderPage(key, recipientCount, result, isError) {
...
async function handleAdminBroadcastGet(req, res) {
    if (!(0, adminAuth_1.requireSubscriberAdmin)(req, res))
        return;
    const stats = (0, db_1.getRealSubscriberStats)();
    const key = (0, adminAuth_1.readSubscriberAdminKey)(req);
    res.type('html').send(renderPage(key, stats.realCount));
}
async function handleAdminBroadcastPost(req, res) {
    if (!(0, adminAuth_1.requireSubscriberAdmin)(req, res))
        return;
    const key = (0, adminAuth_1.readSubscriberAdminKey)(req);
    ...
    if (!dryRun && confirm !== 'SEND') {
        ...
        res
            .type('html')
            .send(renderPage(key, stats.realCount, 'Type <strong>SEND</strong> in the confirm box to deliver this email.', true));
        return;
    }
    const result = await (0, broadcastToSubscribers_1.broadcastToSubscribers)({ subject, message, dryRun });
    if (req.accepts('html') && !String(req.query.format ?? '').includes('json')) {
        ...
        if (dryRun) {
            ...
            text = `Sent <strong>${result.sent}</strong>, failed <strong>${result.failed}</strong>. ${result.failures[0]?.error ? escapeHtml(result.failures[0].error) : ''}`;
        }
